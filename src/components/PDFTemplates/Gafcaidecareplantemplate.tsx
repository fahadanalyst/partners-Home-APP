...
import { BasePDFTemplate, shared, BRAND, CheckSquare } from './BasePDFTemplate';
...
const s = StyleSheet.create({
  ...
  colCheckBoxCell: { flex: 1, alignItems: 'center', justifyContent: 'center' },
...
const CheckItem: React.FC<{ label: string; checked: boolean }> = ({ label, checked }) => (
  <View style={s.checkRow} wrap={false}>
    ...
    <Text style={[shared.value, { fontSize: 8, marginLeft: 5, fontFamily: checked ? 'Helvetica-Bold' : 'Helvetica' }]}>{label}</Text>
  </View>
);
  ...
  return (
    <View style={{ marginBottom: 8 }}>
      ...
      {rows.map(([key, val], idx) => (
        <View key={key} style={idx % 2 === 0 ? s.activityRow : s.activityRowAlt} wrap={false}>
          <Text style={s.colLabel}>{formatKey(key)}</Text>
          <View style={[s.colCheckBoxCell]}><CheckSquare checked={!!val?.cue} /></View>
          <View style={[s.colCheckBoxCell]}><CheckSquare checked={!!val?.assist} /></View>
