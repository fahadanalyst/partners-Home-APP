...
import { PDFHeader, PDFFooter, shared, BRAND, RadioOption, RadioGroup } from './BasePDFTemplate';
...
const YesNoRow: React.FC<{ label: string; value?: string; index: number }> = ({ label, value, index }) => (
  <View style={index % 2 === 0 ? s.yesNoRow : s.yesNoRowAlt} wrap={false}>
    <Text style={s.questionText}>{label}</Text>
    <View style={{ flexDirection: 'row', gap: 10 }}>
      <RadioOption label="YES" selected={value === 'YES'} />
      <RadioOption label="NO" selected={value === 'NO'} />
    </View>
  </View>
);
          ...
          <View style={shared.row3}>
            ...
            <View style={shared.col3}>
              <View style={{ flexDirection: 'row', gap: 16 }}>
                <View style={{ marginBottom: 6 }}>
                  <Text style={shared.label}>Housing Type</Text>
                  <RadioGroup options={['Apt', 'House']} value={data.housingType} />
                </View>
                <View style={{ marginBottom: 6 }}>
                  <Text style={shared.label}>Subsidized?</Text>
                  <RadioGroup options={['Yes', 'No']} value={data.subsidized} />
                </View>
              </View>
            </View>
          </View>
