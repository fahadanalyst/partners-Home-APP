...
import { BasePDFTemplate, shared, BRAND, RadioGroup, RadioCircle } from './BasePDFTemplate';
...
const ADL_LEVELS = ['Independent','Needs Cueing','Needs Assistance','Dependent'];
...
export const GAFCProgressNoteTemplate: React.FC<{ data: any }> = ({ data }) => {
  ...
  return (
    <BasePDFTemplate title="GAFC Progress Note" date={data.visitDate}>
      ...
      <Text style={shared.sectionTitle}>Environment / Safety Check</Text>
      <View style={shared.row2}>
        <View style={shared.col2}><Field label="Cleanliness" value={data.environmentSafety?.cleanliness} /></View>
        <View style={shared.col2}><Field label="Clutter / Hazards" value={data.environmentSafety?.clutterHazards} /></View>
        <View style={shared.col2}><Field label="Functioning Utilities" value={data.environmentSafety?.functioningUtilities} /></View>
        <View style={shared.col2}><Field label="Emergency Plan Awareness" value={data.environmentSafety?.emergencyPlanAwareness} /></View>
      </View>

      {/* Medication Review */}
      <Text style={shared.sectionTitle}>Medication Review</Text>
      <View style={shared.card}>
        <View style={shared.row2}>
          <View style={shared.col2}>
            <Text style={shared.label}>Medications present and labeled</Text>
            <RadioGroup options={['Yes', 'No']} value={data.medicationReview?.presentAndLabeled} />
          </View>
          <View style={shared.col2}>
            <Text style={shared.label}>Participant able to self administer</Text>
            <RadioGroup options={['Yes', 'No']} value={data.medicationReview?.ableToSelfAdminister} />
          </View>
        </View>
        <View style={{ marginTop: 4 }}>
          <Text style={shared.label}>Issues Noted</Text>
          <Text style={shared.value}>{data.medicationReview?.issuesNoted || 'None'}</Text>
        </View>
      </View>
      ...
      <View style={shared.table}>
        <View style={shared.tableHeader}>
          <Text style={[shared.tableHeaderCell, { width: '18%' }]}>Task</Text>
          {ADL_LEVELS.map(level => (
            <Text key={level} style={[shared.tableHeaderCell, { width: '14%', textAlign: 'center' }]}>{level}</Text>
          ))}
          <Text style={[shared.tableHeaderCell, { flex: 1 }]}>Notes</Text>
        </View>
        {ADL_TASKS.map((task, i) => {
          const cell = i % 2 === 0 ? shared.tableCell : shared.tableCellAlt;
          return (
            <View key={task} style={shared.tableRow} wrap={false}>
              <Text style={[cell, { width: '18%', fontFamily: 'Helvetica-Bold' }]}>{task}</Text>
              {ADL_LEVELS.map(level => (
                <View key={level} style={[cell, { width: '14%', alignItems: 'center', justifyContent: 'center' }]}>
                  <RadioCircle selected={data.adls?.[task]?.level === level} />
                </View>
              ))}
              <Text style={[cell, { flex: 1 }]}>{data.adls?.[task]?.notes || '--'}</Text>
            </View>
          );
        })}
      </View>
      ...
      {/* Plan */}
      <Text style={shared.sectionTitle}>Plan</Text>
      <View style={shared.card}>
        <Field label="Follow Up Actions" value={data.plan?.followUpActions} />
        <Field label="Referrals / Coordination" value={data.plan?.referralsCoordination} />
        <Field label="Next Scheduled Visit" value={data.plan?.nextScheduledVisit} />
        <Field label="Participant Instructed to Report" value={data.plan?.participantInstructedToReport} />
      </View>
