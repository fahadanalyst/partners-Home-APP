...
// ─── Form-control primitives (checkboxes / radio buttons) ───────────────────
// These mirror the on-screen form controls so the generated PDF looks the
// same as the web form (all options visible, selected ones marked).
const controlStyles = StyleSheet.create({
  ...
  // Tick drawn with borders (font-safe; '✓' glyph is unreliable in Helvetica)
  tick: {
    width: 5,
    height: 2.5,
    borderLeftWidth: 1.2,
...
/** A single checkbox square (☐ / ☑) drawn natively — no font glyphs. */
export const CheckSquare: React.FC<{ checked?: boolean }> = ({ checked }) => (
  <View style={checked ? controlStyles.boxChecked : controlStyles.box}>
    {checked ? <View style={controlStyles.tick} /> : null}
  </View>
);

/** A single radio circle (○ / ◉) drawn natively. */
export const RadioCircle: React.FC<{ selected?: boolean }> = ({ selected }) => (
  <View style={selected ? controlStyles.circleSelected : controlStyles.circle}>
    {selected ? <View style={controlStyles.dot} /> : null}
  </View>
);

/** Checkbox + label, exactly like a form checkbox row. */
export const CheckOption: React.FC<{ label: string; checked?: boolean }> = ({ label, checked }) => (
  <View style={controlStyles.optionRow} wrap={false}>
    <CheckSquare checked={checked} />
    <Text style={checked ? controlStyles.optionLabelChecked : controlStyles.optionLabel}>{label}</Text>
  </View>
);

/** Radio + label, exactly like a form radio row. */
export const RadioOption: React.FC<{ label: string; selected?: boolean }> = ({ label, selected }) => (
  <View style={controlStyles.optionRow} wrap={false}>
    <RadioCircle selected={selected} />
    <Text style={selected ? controlStyles.optionLabelChecked : controlStyles.optionLabel}>{label}</Text>
  </View>
);

/**
 * Renders ALL checkbox options of a group, ticking the selected ones —
 * mirrors the on-screen checkbox group instead of printing joined text.
 */
export const CheckboxGroup: React.FC<{
  options: string[];
  selected?: string[] | null;
  direction?: 'row' | 'column';
}> = ({ options, selected, direction = 'row' }) => {
  const sel = Array.isArray(selected) ? selected : [];
  return (
    <View style={direction === 'row' ? controlStyles.groupWrap : controlStyles.groupColumn}>
      {options.map((opt) => (
        <CheckOption key={opt} label={opt} checked={sel.includes(opt)} />
      ))}
    </View>
  );
};
...
/**
 * Renders ALL radio options of a group, filling the selected one —
 * mirrors the on-screen radio group instead of printing only the value.
 */
export const RadioGroup: React.FC<{
  options: string[];
  value?: string | null;
  direction?: 'row' | 'column';
}> = ({ options, value, direction = 'row' }) => (
  <View style={direction === 'row' ? controlStyles.groupWrap : controlStyles.groupColumn}>
    {options.map((opt) => (
      <RadioOption key={opt} label={opt} selected={value === opt} />
    ))}
  </View>
);
