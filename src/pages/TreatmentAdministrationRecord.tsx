  ...
  useEffect(() => {
    ...
    const fetchSubmission = async () => {
      try {
        ...
        console.error('TAR: Error fetching submission:', err);
      }
    };
    fetchSubmission();
  }, [editId, reset]);
              ...
              <table className="text-xs border-collapse" style={{ minWidth: 'max-content', width: '100%' }}>
                <thead>
                  <tr className="bg-zinc-50 border-b border-zinc-200">
                    ...
                    <th className="p-2 border-r border-zinc-200 min-w-[240px] text-left">Treatment Description / Frequency</th>
                    ...
                    <tr key={field.id} className="hover:bg-zinc-50/50 transition-colors">
                      ...
                      <td className="p-2 border-r border-zinc-200">
                        <div className="space-y-1">
                          <input {...register(`treatments.${index}.description`)} placeholder="Treatment Description" className="w-full bg-transparent font-bold outline-none text-xs" />
                          <input {...register(`treatments.${index}.frequency`)} placeholder="Frequency" className="w-full bg-transparent text-[10px] text-zinc-500 outline-none border-b border-zinc-100 focus:border-partners-blue py-0.5" />
                        </div>
                      </td>
                      <td className="p-2 border-r border-zinc-200">
                        <input {...register(`treatments.${index}.times`)} placeholder="09:00" className="w-full bg-transparent text-center outline-none font-medium border-b border-zinc-100 focus:border-partners-blue py-0.5" />
                      </td>
                      {Array.from({ length: daysInMonth }).map((_, i) => (
