...
ADD COLUMN IF NOT EXISTS pharmacy_street TEXT,
ADD COLUMN IF NOT EXISTS pharmacy_apt TEXT,
ADD COLUMN IF NOT EXISTS pharmacy_city TEXT,
ADD COLUMN IF NOT EXISTS pharmacy_state TEXT,
ADD COLUMN IF NOT EXISTS pharmacy_zip TEXT,
ADD COLUMN IF NOT EXISTS transportation_company_name TEXT,
ADD COLUMN IF NOT EXISTS transportation_company_phone TEXT,
ADD COLUMN IF NOT EXISTS transportation_company_street TEXT,
ADD COLUMN IF NOT EXISTS transportation_company_apt TEXT,
ADD COLUMN IF NOT EXISTS transportation_company_city TEXT,
ADD COLUMN IF NOT EXISTS transportation_company_state TEXT,
ADD COLUMN IF NOT EXISTS transportation_company_zip TEXT,
ADD COLUMN IF NOT EXISTS adult_day_health_name TEXT,
ADD COLUMN IF NOT EXISTS adult_day_health_phone TEXT,
ADD COLUMN IF NOT EXISTS adult_day_health_street TEXT,
ADD COLUMN IF NOT EXISTS adult_day_health_apt TEXT,
ADD COLUMN IF NOT EXISTS adult_day_health_city TEXT,
ADD COLUMN IF NOT EXISTS adult_day_health_state TEXT,
ADD COLUMN IF NOT EXISTS adult_day_health_zip TEXT,
-- Advanced Directives
...
-- Reload PostgREST schema cache so newly added patient columns are accepted immediately.
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' AND tablename = 'referrals'
      AND policyname = 'Referrals are viewable by authenticated users'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname = 'public' AND tablename = 'referrals'
      AND policyname = 'Referrals are insertable by authenticated users'
  ) THEN
    CREATE POLICY "Referrals are insertable by authenticated users"
      ON referrals FOR INSERT WITH CHECK (auth.role() = 'authenticated');
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname = 'public' AND tablename = 'referrals'
      AND policyname = 'Referrals are updatable by authenticated users'
  ) THEN
    CREATE POLICY "Referrals are updatable by authenticated users"
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname = 'public' AND tablename = 'referrals'
      AND policyname = 'Referrals are deletable by authenticated users'
  ) THEN
    CREATE POLICY "Referrals are deletable by authenticated users"
      ON referrals FOR DELETE USING (auth.role() = 'authenticated');
  END IF;
END $$;
